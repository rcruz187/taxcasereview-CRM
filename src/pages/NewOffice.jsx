...
function TrainingPanel({ firmName }) {
  ...
  // BroadcastChannel — keeps pop-out and this panel in sync
  useEffect(() => {
    const ch = new BroadcastChannel('tcr-screenshare')
    bcRef.current = ch
    ch.addEventListener('message', e => {
      const { type, ...payload } = e.data || {}
      if (type === 'end')            { resetSession() }
  ...
  async function startSession() {
    ...
    setStarting(false)
    // Kick off screen share immediately
    await doStartScreenShare(id)
  }

  async function doStartScreenShare(rid) {
    try {
      ...
      setScreenLabel(surface === 'monitor' ? 'Entire screen' : surface === 'window' ? 'Window' : surface === 'browser' ? 'Browser tab' : track.label?.slice(0,40) || 'Screen')
  ...
  function openPopout() {
    ...
    window.open(url, `tcr-host-${roomId}`, `width=${w},height=${h},left=${left},top=${top},resizable=yes`)
  }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ fontSize: 13, color: 'var(--t3)', lineHeight: 1.6 }}>
          Start a live screen-share training session with <strong style={{ color: 'var(--tx)' }}>{firmName}</strong> staff.
          You share your screen; participants join via a link. Camera + mic included.
        </div>
        <button className="btn pri" onClick={startSession} disabled={starting}
          style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 6 }}>
          {starting ? '…' : '📺 Start training session'}
        </button>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontWeight: 700, color: 'var(--tx)', fontSize: 14 }}>
          {sharingScreen ? `🖥️ Sharing: ${screenLabel}` : 'Session live'}
        </span>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button className="btn sec" onClick={openPopout} style={{ fontSize: 12 }}>⧉ Pop out</button>
          <button className="btn" onClick={endSession}
            style={{ background: '#dc2626', color: '#fff', border: 'none', fontSize: 12 }}>
            End session
          </button>
        </div>
      </div>

      {/* Screen preview — shows the local stream while sharing */}
      <div style={{ borderRadius: 10, overflow: 'hidden', background: '#0f172a',
                    border: '1px solid var(--br)', minHeight: 180 }}>
          ...
          : (
            <div style={{ minHeight: 180, display: 'flex', alignItems: 'center',
              ...
              <span style={{ color: '#475569', fontSize: 13 }}>
                {participants === 0
                  ? 'Share your screen so participants can see it'
                  : 'Screen share stopped — restart below'}
              </span>
            </div>
          )}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {!sharingScreen
          ? <button className="btn pri" onClick={() => doStartScreenShare(roomId)}
              style={{ fontSize: 13 }}>🖥️ Share screen</button>
          : <button className="btn" onClick={() => stopScreenShare(true)}
              style={{ background: '#7c3aed', color: '#fff', border: 'none', fontSize: 13 }}>⏹ Stop sharing</button>
        }
      </div>

      {/* Invite link */}
      <div style={{ background: 'var(--bg)', border: '1px solid var(--br)', borderRadius: 8,
                    padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 10, color: 'var(--t3)', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '.05em' }}>
        ...
        <button className="btn sec" onClick={copyLink} style={{ fontSize: 12, flexShrink: 0 }}>
          📋 Copy
        </button>
      </div>
      ...
      {participants > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '.05em' }}>
            Connected ({participants})
          </div>
          {members.filter(n => n !== 'Me (host)').map(n => (
            <div key={n} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--tx)' }}>
...
function ScreenPreview({ stream }) {
  ...
  return (
    <video ref={ref} autoPlay playsInline muted
      style={{ width: '100%', maxHeight: 280, objectFit: 'contain', display: 'block', background: '#0f172a' }} />
  )
}

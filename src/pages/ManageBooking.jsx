...
// ── Manage a booking (public, tokenized) ──
// Linked from every confirmation email: 🔁 Reschedule / Cancel, Calendly-style.
// All access via SECURITY DEFINER RPCs keyed on the unguessable booking token.

const C = {
  bg: '#0f172a', card: '#1e293b', line: '#334155',
  text: '#f1f5f9', dim: '#94a3b8', accent: '#2563eb',
}

export default function ManageBooking() {
  const { token } = useParams()
  const [params] = useSearchParams()
  const [bk, setBk] = useState(undefined) // undefined loading, null not found
  ...
  const [final, setFinal] = useState(null) // {kind:'canceled'} | {kind:'moved',date,time}
  ...
  async function doCancel() {
    ...
    setBusy(false)
    if (error || !data || data.ok === false) { setErr((data && data.message) || (error && error.message) || 'Could not cancel.'); return }
    sendCancelEmails({ name: bk.client_name, email: bk.contact_email, type: bk.event_type, date: bk.date, time: bk.time, notifyEmail: data.notify_email })
  ...
  async function doReschedule() {
    ...
    setBusy(false)
    if (error || !data || data.ok === false) { setErr((data && data.message) || (error && error.message) || 'That time was just taken — pick another.'); pickDate(date); return }
    sendClientConfirmation({ name: bk.client_name, email: bk.contact_email, type: bk.event_type, date, time, token })
    sendRescheduleNotice({ name: bk.client_name, type: bk.event_type, oldDate: bk.date, oldTime: bk.time, date, time, notifyEmail: data.notify_email })
    setFinal({ kind: 'moved', date, time })
  }

  const box = { background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: 20 }
  const chip = (active) => ({
    padding: '9px 4px', borderRadius: 8, cursor: 'pointer', fontSize: 13, textAlign: 'center', fontWeight: 600,
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: 'system-ui, -apple-system, sans-serif', padding: '32px 16px' }}>
      <div style={{ maxWidth: 560, margin: '0 auto' }}>
        ...
        {bk === undefined ? (
          ...
          <div style={{ ...box, textAlign: 'center', color: C.dim }}>
            This booking link isn't valid anymore. If you need help, just call us or reply to your confirmation email.
          </div>
        ) : final ? (
          <div style={{ ...box, textAlign: 'center' }}>
            <div style={{ fontSize: 38 }}>{final.kind === 'canceled' ? '👍' : '✅'}</div>
            <div style={{ fontWeight: 800, fontSize: 18, marginTop: 8 }}>
              {final.kind === 'canceled' ? 'Appointment canceled' : "You're rescheduled!"}
            </div>
          ...
          <div style={box}>
            ...
            {mode === 'reschedule' && (
              <>
                ...
                {zone !== 'America/New_York' && date && slots && slots.length > 0 && (
                  <div style={{ color: C.dim, fontSize: 11.5, marginBottom: 12 }}>Times shown in your timezone ({zoneShort(zone)}).</div>
                )}
                {err && <div style={{ color: '#fca5a5', fontSize: 13, marginBottom: 10 }}>{err}</div>}

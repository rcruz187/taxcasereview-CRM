...
export default function Email() {
  ...
  async function load() {
    ...
    const [{ data: e }, { data: c }, { data: l }] = await Promise.all([
      ...
      supabase.from('emails').select('id,subject,from_email,from_name,to_email,snippet,body,created_at,is_read,triage,thread_id,mailbox_owner,attachments,client_id,gmail_message_id,in_reply_to').eq('mailbox_owner', user.email).order('created_at', { ascending: false }).limit(300),

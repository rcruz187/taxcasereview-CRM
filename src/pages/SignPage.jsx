    ...
    // Notify the client a signed copy is on file
      ...
      if (doc.client_email) {
        await supabase.functions.invoke('send-email', { body: {
          ...
          html: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px"><div style="text-align:center;margin-bottom:20px"><img src=\"${FIRM.logoUrl}\" alt=\"${FIRM.name}\" style=\"max-height:56px;max-width:190px;object-fit:contain;display:block;margin:0 auto 8px\" onerror=\"this.style.display='none'\"/><div style="font-size:12px;font-weight:800;color:#1d4ed8;letter-spacing:.1em;text-transform:uppercase;margin-top:6px">${FIRM.name}</div></div><p>Dear <strong>${doc.client_name}</strong>,</p><p>Thank you — your signed <strong>${doc.doc_type}</strong> came through on ${signedDate}, and everything below is now on file with us.</p><p>Here is what happens next. Your case has been assigned to a dedicated representative, and we will file your authorization forms with the taxing authorities so we can pull your full account transcripts. That gives us the complete picture of what has been assessed, what has been paid, and where the real exposure is — before we recommend a resolution path.</p><p>You do not need to do anything right now. If the IRS or your state contacts you in the meantime, you can direct them to us. We will reach out as soon as the transcripts are in.</p>${attachmentLinks ? `<p>Please keep these copies for your records:</p><ul style=\"padding-left:20px\">${attachmentLinks}</ul>` : ''}<p>If anything above looks wrong, just reply to this email and we will correct it right away.</p><p style=\"margin-top:20px\">Sincerely,<br/><strong>${FIRM.name}</strong></p><p style="font-size:11px;color:#94a3b8;margin-top:24px;border-top:1px solid #e5e7eb;padding-top:12px">${FIRM.name} · ${FIRM.address}<br/>This message and any attachments are confidential and intended only for the addressee.</p></div>`
        }}).catch(() => {})
      }

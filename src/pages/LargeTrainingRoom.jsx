...
export default function LargeTrainingRoom() {
  ...
  async function getAttendeeToken() {
    const { data, error: fnError } = await supabase.functions.invoke('turn-credentials', {
      body: { action: 'training-join', invite, name: name.trim() },
    })
  ...
  async function joinRoom() {
    ...
    try {
      ...
      await room.join({ sendAudio: isHost, sendVideo: isHost, receiveAudio: true, receiveVideo: true })
      setJoined(true)
  ...
  async function toggleShare() {
    ...
    try {
      if (screenRef.current) {
        await screenRef.current.leave()
        screenRef.current = null
        setSharing(false)
      } else {
        screenRef.current = await room.startScreenShare({ audio: true, video: true })
        setSharing(true)
      }
  ...
  async function toggleRecording() {
    ...
    try {
      if (recordingRef.current) {
        ...
        recordingRef.current = await room.startRecording()
        setRecording(true)
      }
  ...
  async function leaveRoom() {
    ...
    setJoined(false); setSharing(false); setRecording(false)
    ...
    <div style={{ minHeight:'100vh', background:'#080b14', color:'#e2e8f0', display:'flex', flexDirection:'column' }}>
      <header style={{ minHeight:66, padding:'0 22px', borderBottom:'1px solid rgba(99,102,241,.22)', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, background:'rgba(10,15,28,.96)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <img src="/romylabs-logo.png" alt="RomyLabs" style={{ height:34, maxWidth:150, objectFit:'contain' }} onError={e => { e.currentTarget.style.display='none' }} />
          <div><div style={{ fontSize:15, fontWeight:900 }}>RomyLabs Live Training</div><div style={{ fontSize:10, color:'#64748b' }}>{isHost ? 'HOST / PRESENTER' : 'ATTENDEE'} · Room {id}</div></div>
        </div>
      ...
      {!joined ? (
        ...
        <>
          <main style={{ flex:1, minHeight:0, padding:14 }}>
            <div ref={videoRootRef} style={{ width:'100%', height:'100%', minHeight:'calc(100vh - 160px)', background:'#020617', border:'1px solid rgba(148,163,184,.14)', borderRadius:14, overflow:'hidden', position:'relative' }} />
          </main>
          <footer style={{ padding:'12px 18px 16px', display:'flex', justifyContent:'center', gap:9, flexWrap:'wrap', borderTop:'1px solid rgba(99,102,241,.18)', background:'#0a0f1c' }}>
            {isHost && <>
              <button onClick={toggleMic} style={ctrl(micOn)}>{micOn ? '🎤 Mic On' : '🔇 Muted'}</button>
              <button onClick={toggleCamera} style={ctrl(cameraOn)}>{cameraOn ? '📹 Camera On' : '📷 Camera Off'}</button>
              <button onClick={toggleShare} style={ctrl(sharing)}>{sharing ? '🛑 Stop Share' : '🖥️ Share Screen'}</button>
              <button onClick={toggleRecording} style={ctrl(recording)}>{recording ? '⏹ Stop Recording' : '⏺ Record'}</button>
            </>}

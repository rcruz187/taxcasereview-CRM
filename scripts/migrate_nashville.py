...
while True:
    ...
    if isinstance(result, dict) and result.get('message'):
        print(f"  Bulk error: {result['message'][:150]}")
        # Row by row fallback
        ok = 0
        for row in rows:
            single = f"INSERT INTO {TABLE} ({col_str}) VALUES ({','.join(val(row[c]) for c in cols)}) ON CONFLICT DO NOTHING"
            r = api_query(NAS, single)
            if not (isinstance(r, dict) and r.get('message')): ok += 1
        print(f"  Row fallback: {ok}/{len(rows)} succeeded")
    else:
        print(f"  offset={offset} inserted {len(rows)} rows")
...
if count and isinstance(count, list):
    print(f"Nashville {TABLE}: {count[0].get('cnt','?')} rows")

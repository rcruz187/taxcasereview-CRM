...
async function loadTemplateBytes(rawPath) {
  ...
  if(/^https?:\/\//i.test(raw)){
    ...
    const type=(res.headers.get('content-type')||'').toLowerCase(),bytes=new Uint8Array(await res.arrayBuffer())
  ...
  const candidates=[path.join(root,'public',rel),path.join(root,'public','templates',path.basename(rel)),path.join(root,'public','irs-forms',path.basename(rel)),path.join(root,rel)]
...
const manualOnly=irsSrc.includes('export const MANUAL_ONLY_FORM_TYPES') ? new Set(evalLiteral(captureArray(irsSrc,'export const MANUAL_ONLY_FORM_TYPES'))) : new Set()
let hardFailures=[]
console.log('=== IRS TEMPLATE / FIELD AUDIT ===')
for(const [type,map] of Object.entries(fieldMaps)){
  if(manualOnly.has(type)){console.log(`${type}: MANUAL_ONLY fail-closed (no autofill/e-sign generation)`);continue}
  const relRaw=templatePaths[type]
  if(!relRaw){hardFailures.push(`IRS ${type}: no TEMPLATE_PATHS entry`);continue}
  try{
    const bytes=await loadTemplateBytes(relRaw),pdf=await PDFDocument.load(bytes,{ignoreEncryption:true})
    const actual=pdf.getForm().getFields().map(f=>f.getName()),names=new Set(actual)
    const mapped=Object.entries(map).filter(([k,v])=>k!=='idType'&&typeof v==='string'),missing=mapped.filter(([,v])=>!names.has(v))
    ...
    if(missing.length){console.log(`  ACTUAL_FIELDS ${JSON.stringify(actual)}`);hardFailures.push(`IRS ${type}: ${missing.length} mapped PDF fields missing`)}
  }catch(e){hardFailures.push(`IRS ${type}: ${e.message}`)}
}
console.log(`IRS manual-only forms: ${[...manualOnly].join(', ')||'none'}`)
if(!manualOnly.has('4549')) hardFailures.push('IRS 4549: must remain explicit manual-only while IRS provides no public standalone PDF')
...
for(const f of stateForms){
  try{
    const remote=/^https?:\/\//i.test(String(f.url)),bytes=await loadTemplateBytes(f.url)
    if(remote)remoteStateForms++
    const pdf=await PDFDocument.load(bytes,{ignoreEncryption:true});let count=0
...
for(const key of ['pdf_attachments','createSignedUrl','documents','signature','signed','certificate'])console.log(`${key}: ${signSrc.includes(key)?'YES':'NO'}`)

if(hardFailures.length){console.error('\nHARD FAILURES:');hardFailures.forEach(x=>console.error('- '+x));process.exitCode=1}

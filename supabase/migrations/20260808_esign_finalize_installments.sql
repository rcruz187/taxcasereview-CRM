-- Patch esign_finalize to auto-create payment installments when Service Addendum is signed
-- Adds one line after the v_target_status advance block
...
BEGIN
  ...
  BEGIN
    ...
    IF v_target_status IS NOT NULL AND v_lead_id IS NOT NULL THEN
      ...
      END IF;
    END IF;
    -- AUTO-CREATE PAYMENT INSTALLMENTS when addendum is signed

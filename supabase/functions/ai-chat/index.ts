...
- Analyzing IRS transcripts — TC codes, CSED dates, assessment history, balance breakdown, lien/levy status
- Analyzing tax returns — identifying issues, missed deductions, unreported income, filing errors
- Call transcript analysis — summarizing calls, extracting action items, identifying client concerns
...
async function getWeather(query: string): Promise<string | null> {
  try {
    ...
    const res = await fetch(`https://wttr.in/${encoded}?format=j1`, { headers: { 'User-Agent': 'TaxResCRM/1.0' } })
    ...
    const tempF = current.temp_F, feelsF = current.FeelsLikeF, humidity = current.humidity, windMph = current.windspeedMiles
    const areaName = area?.areaName?.[0]?.value || location
    const region = area?.region?.[0]?.value || ''
    const today = data.weather?.[0]
    const maxF = today?.maxtempF, minF = today?.mintempF
...
async function webSearch(query: string): Promise<string | null> {
  try {
    ...
    const res = await fetch(`https://api.duckduckgo.com/?q=${encoded}&format=json&no_html=1&skip_disambig=1`, { headers: { 'User-Agent': 'TaxResCRM/1.0' } })
    if (!res.ok) return null
    const data = await res.json()
    const parts: string[] = []
    ...
    for (const t of topics) { if (t.Text) parts.push(t.Text) }
...
serve(async (req) => {
  ...
  try {
    ...
    if (needsLiveData(message)) {
      ...
      if (!liveData) liveData = await webSearch(message)
    }
    ...
    const finalMessage = liveData ? `${message}\n\n[Live data retrieved]:\n${liveData}` : message

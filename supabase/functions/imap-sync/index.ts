...
//
// Stalwart Mail Server exposes IMAP on port 993 (SSL/TLS) and SMTP on
// ports 587 (STARTTLS) / 465 (SSL). We connect via Deno TLS.
...
function configured() {
  return Boolean(SUPABASE_URL && SERVICE_KEY && ENCRYPT_KEY)
}
...
function extractEmailAddress(headerValue: string): string {
  ...
  return match?.[1]?.toLowerCase().trim() || value.toLowerCase().trim()
}
...
async function matchToClient(supabase:any, senderEmail: string, tenantId: string): Promise<{ clientId: string | null, clientName: string | null }> {
  const { data: client } = await supabase.from('clients').select('id, name').eq('email', senderEmail).eq('tenant_id', tenantId).maybeSingle()
  if (client) return { clientId: client.id, clientName: client.name }
  const { data: lead } = await supabase.from('leads').select('id, name').eq('email', senderEmail).eq('tenant_id', tenantId).maybeSingle()
  return { clientId: lead?.id || null, clientName: lead?.name || null }
}

async function syncAccount(supabase:any, account: Record<string, any>): Promise<{ synced: number, errors: string[] }> {
  const errors: string[] = []
  let synced = 0
  const { data: decrypted, error: decErr } = await supabase.rpc('decrypt_email_password', { p_encrypted: account.encrypted_password, p_key: ENCRYPT_KEY })
  ...
  try {
    ...
    const loginResp = await imapCommand(conn, 'A1', `LOGIN "${String(account.email_address||'').replace(/["\\]/g,'')}" "${String(decrypted).replace(/["\\]/g,'')}"`)
    if (!loginResp.includes('A1 OK')) throw new Error(`IMAP LOGIN failed: ${loginResp.slice(0, 200)}`)
    ...
    if (seqNums.length === 0) { await imapCommand(conn, 'A99', 'LOGOUT'); return { synced:0, errors:[] } }

    let tag = 10
    for (const seq of seqNums.slice(0, 50)) {
      try {
        const fetchResp = await imapCommand(conn, `B${tag}`, `FETCH ${seq} (BODY[])`); tag++
        const { subject, from, date, messageId } = parseEnvelope(fetchResp)
        const senderEmail = extractEmailAddress(from)
        const { text, html } = extractBody(fetchResp)
        const { data: existing } = await supabase.from('emails').select('id').eq('message_id', messageId).eq('tenant_id', account.tenant_id).maybeSingle()
        if (existing) continue
        const { clientId, clientName } = await matchToClient(supabase,senderEmail,String(account.tenant_id))
        const threadId = String(messageId).split('@')[0] || String(messageId)
        const parsedDate=new Date(date)
        await supabase.from('emails').insert([{
          tenant_id: account.tenant_id, email_account_id: account.id, message_id: messageId, thread_id: threadId,
          mailbox_owner: account.employee_email, sender: senderEmail, from_address: senderEmail,
          recipients: [{ email: String(account.email_address||'') }], subject, body:text, body_html:html,
          direction:'inbound', triage:'Inbox', status:'Received', is_read:false,
          received_at: Number.isNaN(parsedDate.getTime()) ? new Date().toISOString() : parsedDate.toISOString(),
          client_id: clientId, clientName, created_at:new Date().toISOString(),
        }])
        synced++
      } catch (msgErr) { errors.push(`Seq ${seq}: ${(msgErr as Error).message}`) }
    }
...
serve(async (req) => {
  ...
  if (!configured()) return new Response(JSON.stringify({ok:false,error:'Server email encryption is not configured'}),{status:500,headers:{...corsHeaders,'Content-Type':'application/json'}})
  try {
    ...
    for (const account of (accounts || [])) {
      const { synced, errors } = await syncAccount(supabase,account)
      await supabase.from('email_sync_log').insert([{account_id:account.id,tenant_id:account.tenant_id,messages_new:synced,status:errors.length?'error':'ok',error_message:errors.length?errors.join('; '):null}])
      await supabase.from('email_accounts').update({last_sync_at:new Date().toISOString(),sync_status:errors.length?'error':'ok',sync_error:errors.length?errors[0]:null}).eq('id',account.id).eq('tenant_id',account.tenant_id)

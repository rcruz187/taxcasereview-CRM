...
function h(headers: any[], name: string) {
  return headers?.find((x: any) => String(x.name).toLowerCase() === name.toLowerCase())?.value || ''
}
function address(v = '') {
  const m = v.match(/<([^>]+)>/)
  return (m ? m[1] : v).trim()
...
function displayName(v = '') {
  const m = v.match(/^"?([^"<]*)"?\s*<[^>]+>$/)
  return m?.[1]?.trim() || address(v)
}
function part(payload: any, mime: string): string | null {
  if (!payload) return null
  if (payload.mimeType === mime && payload.body?.data) return payload.body.data
  for (const p of payload.parts || []) { const x = part(p, mime); if (x) return x }
  return null
}
function plain(html = '') {
  return html.replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '').replace(/<[^>]+>/g, ' ').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim()
}
function files(payload: any, out: any[] = []) {
  if (!payload) return out
  if (payload.filename && payload.body?.attachmentId) out.push({ filename: payload.filename, mimeType: payload.mimeType || 'application/octet-stream', size: payload.body.size || 0, attachmentId: payload.body.attachmentId })
  for (const p of payload.parts || []) files(p, out)
  return out
}
...
async function token(db: any, acct: any, creds: any) {
  ...
  const res = await fetch(TOKEN_URL, {
    ...
    body: new URLSearchParams({ refresh_token: acct.gmail_refresh_token, client_id: creds.gmail_client_id, client_secret: creds.gmail_client_secret, grant_type: 'refresh_token' }),
  })
...
async function listIds(tok: string, label: string, opts: { pages?: number, q?: string, pageSize?: number } = {}) {
  ...
  for (let page = 0; page < pages; page++) {
    const qs = new URLSearchParams({ maxResults: String(opts.pageSize || 500), labelIds: label })
    if (pageToken) qs.set('pageToken', pageToken)
    if (opts.q) qs.set('q', opts.q)
    const res = await fetch(`${GMAIL}?${qs}`, { headers: { Authorization: `Bearer ${tok}` } })
    const data = await res.json().catch(() => ({}))
...
async function parse(tok: string, id: string, clients: any[], leads: any[]) {
  const res = await fetch(`${GMAIL}/${encodeURIComponent(id)}?format=full`, { headers: { Authorization: `Bearer ${tok}` } })
  const msg = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(msg?.error?.message || `Gmail get failed (${res.status})`)
  const labels = msg.labelIds || []
  const sent = labels.includes('SENT')
  const inbox = labels.includes('INBOX')
  if (!sent && !inbox) return null
  const headers = msg.payload?.headers || []
  const fromH = h(headers, 'From'), toH = h(headers, 'To')
  const counterpartH = sent ? toH : fromH
  ...
  const htmlData = part(msg.payload, 'text/html')
  const html = htmlData ? decode(htmlData) : null
  const textData = part(msg.payload, 'text/plain')
  const body = textData ? decode(textData) : html ? plain(html) : (msg.snippet || '')
  const dateH = h(headers, 'Date')
  const at = dateH && !Number.isNaN(Date.parse(dateH)) ? new Date(dateH).toISOString() : new Date(Number(msg.internalDate || Date.now())).toISOString()
  return {
    row: {
      recipient: sent ? counterpart : address(fromH), clientName: client?.name || lead?.name || displayName(counterpartH) || counterpart,
      subject: h(headers, 'Subject') || '(no subject)', body, body_html: html, triage: sent ? 'Sent' : 'Inbox', status: sent ? 'Sent' : 'Received',
      gmail_message_id: msg.id, gmail_thread_id: msg.threadId, from_address: address(fromH), received_at: at, created_at: at,
      is_read: sent || !labels.includes('UNREAD'), attachments: files(msg.payload),
    },
...
async function allLocalGmailRows(db: any, owner: string) {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await db.from('emails').select('id,gmail_message_id,is_read,triage').eq('mailbox_owner', owner).is('deleted_at', null).not('gmail_message_id', 'is', null).order('id', { ascending: true }).range(from, from + 999)
...
async function reconcileUnread(db: any, tok: string, owner: string) {
  const unreadIds = await listIds(tok, 'UNREAD', { pages: 20, pageSize: 500 })
  const unread = new Set(unreadIds)
  const rows = await allLocalGmailRows(db, owner)
  let changed = 0
  for (let i = 0; i < rows.length; i += 300) {
    const chunk = rows.slice(i, i + 300)
    const toUnread = chunk.filter((r: any) => unread.has(r.gmail_message_id) && r.is_read !== false).map((r: any) => r.id)
    const toRead = chunk.filter((r: any) => !unread.has(r.gmail_message_id) && r.is_read !== true).map((r: any) => r.id)
    if (toUnread.length) { const { error } = await db.from('emails').update({ is_read: false }).in('id', toUnread); if (error) throw error; changed += toUnread.length }
    if (toRead.length) { const { error } = await db.from('emails').update({ is_read: true }).in('id', toRead); if (error) throw error; changed += toRead.length }
  }
...
async function importIds(db: any, tok: string, owner: string, tenantId: string, ids: string[], clients: any[], leads: any[]) {
  ...
  for (let i = 0; i < ids.length; i += 100) {
    ...
    const { data: knownRows } = await db.from('emails').select('gmail_message_id').eq('mailbox_owner', owner).in('gmail_message_id', batch)
    const known = new Set((knownRows || []).map((r: any) => r.gmail_message_id))
    for (const id of batch.filter(x => !known.has(x))) {
      try {
        const parsed = await parse(tok, id, clients, leads)
        if (!parsed) continue
        const { error } = await db.from('emails').insert({ ...parsed.row, tenant_id: tenantId, mailbox_owner: owner })
        if (error) { console.error('gmail-sync insert', id, error.message); continue }
        inserted++
        ...
        if (m) {
          ...
          const note = `📧 Email ${direction} — "${parsed.row.subject}"${preview ? `\n${preview}${String(parsed.row.body || '').length > 120 ? '…' : ''}` : ''}`
          if (m.kind === 'client') await db.from('client_notes').insert({ clientname: m.name, text: note, note_type: 'Email', author: direction === 'Sent' ? owner : m.name, created_at: parsed.row.created_at, tenant_id: tenantId })
          else await db.from('lead_notes').insert({ lead_id: m.id, lead_name: m.name, text: note, type: 'Email', author: direction === 'Sent' ? owner : m.name, created_at: parsed.row.created_at, tenant_id: tenantId })
        }
      } catch (e) { console.error('gmail-sync message import', id, e) }
    }
  }
  return inserted
}
...
async function syncAccount(db: any, acct: any, tenantId: string, creds: any) {
  ...
  const [{ data: clients }, { data: leads }] = await Promise.all([
    db.from('clients').select('id,name,email').eq('tenant_id', tenantId),
    db.from('leads').select('id,name,email').eq('tenant_id', tenantId),
  ])
  const backfillDone = acct.gmail_backfill_phase === 'done'
  const query = backfillDone ? '' : `after:${Math.floor((Date.now() - 365 * 86400000) / 1000)}`
  const pages = backfillDone ? 1 : 10
  let inserted = 0
  for (const label of ['INBOX','SENT']) {
    const ids = await listIds(tok, label, { pages, pageSize: backfillDone ? 100 : 500, q: query || undefined })
    inserted += await importIds(db, tok, acct.employee_email, tenantId, ids, clients || [], leads || [])
  }
  const unread = await reconcileUnread(db, tok, acct.employee_email)
  ...
  await db.from('emails').delete().lt('created_at', cutoff).eq('mailbox_owner', acct.employee_email)
...
async function messageAction(req: Request, db: any, payload: any) {
  ...
  const { data: rows, error } = await db.from('emails').select('id,gmail_message_id,mailbox_owner,tenant_id').in('id', ids)
  if (error) throw error
  ...
  if (!emp?.tenant_id || String(emp.status || 'Active').toLowerCase() !== 'active') return json({ ok: false, error: 'Employee is not active' }, 403)
  ...
  let tok = ''
  if (gmailRows.length) {
    const { data: acct } = await db.from('employee_gmail_accounts').select('employee_email,gmail_refresh_token,gmail_access_token,gmail_token_expiry').ilike('employee_email', user.email).limit(1).maybeSingle()
    const { data: creds } = await db.from('settings').select('gmail_client_id,gmail_client_secret').eq('tenant_id', tenantId).limit(1).maybeSingle()
...
serve(async req => {
  ...
  try {
    let payload: any = {}
    if (req.method === 'POST' && (req.headers.get('content-type') || '').includes('application/json')) payload = await req.json().catch(() => ({}))
    ...
    const [{ data: settings }, { data: accounts }] = await Promise.all([
      db.from('settings').select('tenant_id,gmail_client_id,gmail_client_secret').not('gmail_client_id','is',null),
      db.from('employee_gmail_accounts').select('employee_email,gmail_refresh_token,gmail_access_token,gmail_token_expiry,gmail_backfill_phase').not('gmail_refresh_token','is',null),
    ])
    if (!accounts?.length) return json({ ok: true, synced: 0, failed: 0, inserted: 0, unreadChanged: 0 })
    const credsByTenant = new Map((settings || []).filter((r: any) => r.tenant_id && r.gmail_client_id && r.gmail_client_secret).map((r: any) => [r.tenant_id, r]))
    const emails = accounts.map((a: any) => a.employee_email)
    const { data: employees } = await db.from('employees').select('email,tenant_id').in('email', emails)
    ...
    for (const acct of accounts) {
      ...
      const creds = tenantId ? credsByTenant.get(tenantId) : null
      if (!tenantId || !creds) {
        ...
        await db.from('employee_gmail_accounts').update({ gmail_last_error: msg }).eq('employee_email', acct.employee_email)
        continue
      }
      try {
        const result = await syncAccount(db, acct, tenantId, creds)
        inserted += result.inserted; unreadChanged += result.changed; synced++
      } catch (e) {

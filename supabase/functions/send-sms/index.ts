...
serve(async(req)=>{
 ...
 try{
  ...
  const admin=createClient(url,service),payload=await req.json();let {to,body,lead_id,client_id,user_id,employee_portal_token}=payload
  ...
  if(!tenantId&&payload.kind==='esign_signed_receipt'&&payload.esign_id){
    ...
    if(!e||e.status!=='Signed')return new Response(JSON.stringify({error:'Invalid signing request'}),{status:403,headers:{...corsHeaders,'Content-Type':'application/json'}})
    if(e.signed_sms_sent_at)return new Response(JSON.stringify({success:true,already_sent:true}),{headers:{...corsHeaders,'Content-Type':'application/json'}})
    if(!e.client_phone)return new Response(JSON.stringify({error:'Signing request has no phone'}),{status:422,headers:{...corsHeaders,'Content-Type':'application/json'}})
  ...
  const{error:logErr}=await admin.from('sms_messages').insert({clientName,phone:String(to),body:String(body).trim(),status:sw.status||'sent',direction:'outbound',signalwire_sms_id:sw.sid||null,sent_by:sentBy,tenant_id:tenantId,client_id:client_id?String(client_id):null,read:true});if(logErr)console.error('[send-sms] log failed',logErr.message)
